...
<div class="container">
    <div class="row">
        @foreach ($pagecontent as $page)
            <p>{!!$page->main_text!!}</p>
            <div class="row">
                @foreach($page->ColomContext->slice(($page->ColomContext->count() / 2), $page->ColomContext->count()) as $p)
            ...
            <div class="row">
                @foreach($page->ColomContext->slice(0, ($page->ColomContext->count() / 2)) as $p)
                    @if($page->ColomContext->count() > 1)
                        <div class="col-md-6">
                            <h2>{{$p->colom_title_text}}</h2>
                            <p>{!!$p->colomn_text!!}</p>
