...
@if ($errors->any())
    <div class="alert alert-danger">
        <strong>Whoops!</strong> Er gaat iets mis zie de errormessage<br><br>
        <ul>
...
@endif
<section class="content"> @foreach ($pagecontent as $page) <div class="pull-right ">
    <a class="btn btn-danger" href="{{ route('youtubeWebpage.editYoutube', $page->id) }}"> Back</a>
    <button type="submit" form="submition-form" formaction="{{ route('imageWebpage.updateImage', $page->id) }}" class="btn btn-primary">Update</button>
    <input type="button" name="add" value="nieuwe afbeelding toevoegen" id="addRemoveIp" class="btn btn-outline-dark" required>
  </div>
  <div class="col-12 mt-4">
    <div class="card">
      <form id="submition-form" method="POST"> @csrf @method('POST') <div class="table-responsive">
                    ...
                    <td>
                        <select id="images" name="oldInput[{{$img->id }}][image_id]" class="w-100">
                            @foreach($afbeeldingen as $af)
                                <option value="{{$af->id}}" @if($af->id == $img->id) selected @endif>{{$af->title}}</option>
                            @endforeach
                        </select>
                    </td>
              </tr>
